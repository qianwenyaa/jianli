      ...
      <el-row>
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.jianli" label="简历" prop="jianli">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.jianli.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="反馈" prop="fankui">
          <el-input v-model="ruleForm.fankui" 
              placeholder="反馈" clearable  :readonly="ro.fankui"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="反馈" prop="fankui">
              <el-input v-model="ruleForm.fankui" 
                placeholder="反馈" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `jianlishangchuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jianlishangchuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
