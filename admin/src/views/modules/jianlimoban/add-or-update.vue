<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="模板名称" prop="mobanmingcheng">
          <el-input v-model="ruleForm.mobanmingcheng" 
              placeholder="模板名称" clearable  :readonly="ro.mobanmingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="模板名称" prop="mobanmingcheng">
              <el-input v-model="ruleForm.mobanmingcheng" 
                placeholder="模板名称" readonly></el-input>
                                    ...
                                    <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="模板类型" prop="mobanleixing">
          <el-select v-model="ruleForm.mobanleixing" placeholder="请选择模板类型">
            <el-option
                v-for="(item,index) in mobanleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="模板类型" prop="mobanleixing">
	      <el-input v-model="ruleForm.mobanleixing"
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          	            if(o=='mobanmingcheng'){
            this.ruleForm.mobanmingcheng = obj[o];
	    this.ro.mobanmingcheng = true;
            continue;
          }
	            	            if(o=='mobanleixing'){
            this.ruleForm.mobanleixing = obj[o];
	    this.ro.mobanleixing = true;
            continue;
          }
	            	            if(o=='jianjie'){
	...
	this.ruleForm.jianjie = this.ruleForm.jianjie.replace(reg,'../../../springbootz9481/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `jianlimoban/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jianlimobanCrossAddOrUpdateFlag = false;
                  this.parent.search();
